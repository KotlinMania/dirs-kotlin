...
kotlin {
    ...
    val xcf = XCFramework("Dirs")

    macosArm64 {
        binaries.framework { baseName = "Dirs"; xcf.add(this) }
    }
    iosArm64 {
        binaries.framework { baseName = "Dirs"; xcf.add(this) }
    }
    ...
    swiftExport {
        moduleName = "Dirs"
        flattenPackage = "io.github.kotlinmania.dirs"
    }

    android {
        namespace = "io.github.kotlinmania.dirs"
        compileSdk = 34
        minSdk = 24
...
mavenPublishing {
    ...
    coordinates(group.toString(), "dirs-kotlin", version.toString())

    pom {
        name.set("dirs-kotlin")
        description.set("Kotlin Multiplatform port of soc/dirs-rs - A tiny low-level library that provides platform-specific standard locations of directories for config, cache and other data on Linux, Windows, macOS and Redox by leveraging the mechanisms defined by the XDG base/user directory specifications on Linux, the Known Folder API on Windows, and the Standard Directory guidelines on macOS")
        inceptionYear.set("2026")
        url.set("https://github.com/KotlinMania/dirs-kotlin")
        ...
        scm {
            url.set("https://github.com/KotlinMania/dirs-kotlin")
            connection.set("scm:git:git://github.com/KotlinMania/dirs-kotlin.git")
            developerConnection.set("scm:git:ssh://github.com/KotlinMania/dirs-kotlin.git")
        }
    }
}
...
val codeqlSourceClasspath: Configuration by configurations.creating {
    description = "Runtime classpath for CodeQL extraction of Kotlin sources"
    isCanBeResolved = true
    isCanBeConsumed = false
}
...
val codeqlCompileJvm = tasks.register<JavaExec>("codeqlCompileJvm") {
    description =
        "Compile Kotlin sources with kotlinc 2.3.21 for CodeQL Java/Kotlin extraction."
    group = "verification"
    ...
    doFirst {
        ...
        if (sourceFiles.isEmpty()) {
            val sentinelFile = sentinelDir.get().asFile.resolve("io/github/kotlinmania/dirs/codeql/CodeqlEmptySentinel.kt")
            sentinelFile.parentFile.mkdirs()
            sentinelFile.writeText(
                """
                // Auto-generated. Present so codeqlCompileJvm has at least
                // one Kotlin source to feed kotlinc; replaced by real
                // sources once porting begins.
                package io.github.kotlinmania.dirs.codeql
