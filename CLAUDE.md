# Claude Code Project Instructions — dirs-kotlin

## Project Overview

This is **dirs-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`dirs`](https://crates.io/crates/dirs).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/dirs/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/dirs>
